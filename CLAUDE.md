...
- No passwords are ever stored. Auth is via session cookies captured through a one-time live login, using the `capture-cookies` skill (a standalone `recapture.py` script used to exist for this but was removed — see "Session-expiry detection" below).
- Whenever a run confirms the session is authenticated (the real dashboard renders, not the guest stub — tracked by `authenticated` in `run_for_account`), the script re-saves that account's current cookies back to its GitHub secret (`persist_refreshed_cookies()`). **This is required, not optional**: GoMining rotates its `refresh_token` on use, so a static cookie saved once will work exactly once and then permanently fail. It fires on `authenticated`, not on the narrower "tap fully succeeded" (`success`), so a run that authenticates fine but then fails for an unrelated reason (the click never registers into cooldown, a later retry hits a load error) still saves the just-rotated cookies instead of stranding them — added 2026-09-17 after a run's cookies went stale between a good save and the next scheduled attempt.
...
**Recovery:** use the `capture-cookies` skill (drives a visible browser via the Playwright MCP; the account owner completes the "Continue with Google" step, Claude pushes the fresh cookies straight to the `GOMINING_COOKIES_<LABEL>` secret). A standalone `recapture.py` script used to do this without Claude in the loop, but Google now reliably blocks scripted logins to its own sign-in page ("This browser or app may not be secure") regardless of what automation tool drives them, so the script was removed 2026-09-17 — this is Google's own anti-automation policy, not something fixable on our end.

**2FA and this automation:** enabling 2FA (on the GoMining account or the Google account it signs in with) does **not** affect the nightly run on an ongoing basis — the run reuses a saved session and never hits a login form, so no 2FA prompt is ever reached. It will, however, invalidate the current session *once* when first turned on, producing a single "session expired" run; the `capture-cookies` skill fixes it and subsequent runs are normal. The only thing 2FA genuinely rules out is *fully scripting* the re-login step, which is manual by design anyway.
...
- `GOMINING_COOKIES_<LABEL>` (one per account in `GOMINING_ACCOUNT_LABELS`): session cookies (JSON array), self-refreshed by the script every run. Use the `capture-cookies` skill to re-capture from scratch after a "session expired".
...
GitHub emails on failure, and (if `SENTRY_DSN` is set) so does Sentry's default high-priority-issue alert rule — so a "session expired" shouldn't sit unnoticed as long as Sentry email notifications are on for your account. Check the run log first: a `[LABEL] FAILED: session expired` line means that account's saved session is dead — use the `capture-cookies` skill to get a fresh one. GoMining invalidates sessions on their side periodically, so this is expected occasionally, not a bug. Failures from any *other* cause also upload a screenshot + HTML snapshot as a workflow artifact (`debug-artifacts`); a "session expired" return does not (it's self-explanatory, and skips straight past the artifact-saving path).
